...
def createTermProbability():
    ...
    for key, values in PosTermFreq.items():
        if isLegalTermCount(key) :
            ...
            TempProb = Decimal(((PosTermFreq[key]*1.0/PosLen)*(1-smoothingParameter))+(smoothingParameter*SmoothingProb[key]))
            #print TempProb
    ...
    for key, values in NegTermFreq.items():
        if isLegalTermCount(key) :
            ...
            TempProb = Decimal(((NegTermFreq[key]*1.0/NegLen)*(1-smoothingParameter))+(smoothingParameter*SmoothingProb[key]))
            #print TempProb
